# ============================================================================
# ⚠️  ABANDONED / STALE — DO NOT RUN.  (marked 2026-06-22)
# Frozen older duplicate of recording_grid.py carrying the OLD pre-cello-move
# calibration: old RETRACT_DIR_BASE [0.68861,0.031962,0.724428] (~66° off the
# current normal), old A-string waypoints, and the retired CONTACT_ANCHORS model.
# Running it would command motion in the WRONG press direction.
# LIVE module = recording_grid.py. (The __main__ below refuses to run.)
# ============================================================================
"""
recording_script.py
...
# Add project root (two levels up from SoundClassifier/Data_Collection/) so the
# BaselineControls package is importable regardless of where this is launched.
...
FROG_TARGET_STR = np.array([.336637615375, .773335607743, .103937252349,
                            -1.369835518384, -2.336199267621, 1.326965437172])
TIP_TARGET_STR  = np.array([.525205911288, .350983193771, .214779688012,
...
# Found empirically with find_liftoff.py (full retract->travel->descend cycle
# watched on the real robot, with the validated paganini A-string waypoints).
# This direction is strongly upward (base-z ≈ 0.72) and lifts the bow cleanly
# off the string without dragging during the lateral travel.
#       RETRACT_DIR_BASE = [0.68861, 0.031962, 0.724428]   (lift ~50-60mm)
# The reach problem near the frog is handled by clamp_reach (below), NOT by
# bending this direction. In the tip half of the bow (incl. the upper_middle
# test zone) this retract is well within reach; only the frog zone clamps.
...
def retract(ctrl):
    """
    Retract the bow off the string along the calibrated retract direction to the
    clearance distance, from wherever it currently is. Blocking.
    """
    pose      = np.array(ctrl.rtde_r.getActualTCPPose())
    retracted = offset_along_normal(pose, LIFT_DISTANCE)
...
def bow_pose_full(frac):
    ...
    return FROG_TARGET_STR + frac * (TIP_TARGET_STR - FROG_TARGET_STR)
...
def execute_stroke_force_mode(ctrl: TargetStringBaselineController, note: dict):
    ...
    return ctrl.state_logger.get_summary(t_start=bow_t_start, t_end=bow_t_end)
...
if __name__ == '__main__':
    import sys as _sys
    _sys.exit("recording_script.py is ABANDONED/STALE (old 2026-06 calibration, old normal) — "
              "refusing to run. Use recording_grid.py for live recording.")
